...
def collab():
    ...
    if request.method == "POST":
        ...
        search_results = f"You searched result for: {user_input}"
        return render_template("collab.html", user_input=user_input, random_sample=recommended_rows, search_results=search_results, data=data)
    else:
        random_sample = data.sample(n=10)  # Get a random sample of 10 rows
        return render_template("collab.html", user_input=user_input, search_results=search_results, random_sample=random_sample)
